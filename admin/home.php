    ...
    <div class="info-box">
      ...
      <div class="info-box-content">
        ...
        <span class="info-box-number text-right">
          ...
          if ($today_budget !== null) {
            echo number_format((float)$today_budget, 2); // Cast to float and display with 2 decimal places
          } else {
            echo "0.00"; // Handle the case where $today_budget is null
          }
          ?>
  ...
  <div class="col-12 col-sm-6 col-md-3">
    <div class="info-box mb-3">
      ...
      <div class="info-box-content">
        ...
        <span class="info-box-number text-right">
          <?php
          $today_budget_result = $conn->query("SELECT sum(amount) as total FROM `running_balance` WHERE category_id IN (SELECT id FROM categories WHERE status = 1) AND date(date_created) = '" . (date("Y-m-d")) . "' AND balance_type = 1 ");
          $today_budget_data = $today_budget_result->fetch_assoc();
          $today_budget = isset($today_budget_data['total']) ? (float)$today_budget_data['total'] : 0.0;
          echo number_format($today_budget, 2); // Display with 2 decimal places
          ?>
    ...
    <div class="info-box mb-3">
      ...
      <div class="info-box-content">
        ...
        <span class="info-box-number text-right">
          ...
          $today_expense_result = $conn->query("SELECT sum(amount) as total FROM `running_balance` WHERE category_id IN (SELECT id FROM categories WHERE status = 1) AND date(date_created) = '" . (date("Y-m-d")) . "' AND balance_type = 2 ");
          $today_expense_data = $today_expense_result->fetch_assoc();
          $today_expense = isset($today_expense_data['total']) ? (float)$today_expense_data['total'] : 0.0;
          echo number_format($today_expense, 2); // Display with 2 decimal places
